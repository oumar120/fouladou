...
<div class="super_container">
  ...
  <br/>
    ...
    if(!empty($_GET['search'])){
     ...
     $req=$bdd->prepare('select * from music where nom like ? OR titre like ? OR album like ?');
     $req->execute(array('%'.$search.'%','%'.$search.'%','%'.$search.'%'));
     $count=$req->rowCount();
       $reponse=$bdd->prepare('select * from music where nom like ? OR titre like ? OR album like ? limit 0,10');
     $reponse->execute(array('%'.$search.'%','%'.$search.'%','%'.$search.'%'));
     if ($count>=1) {
       echo "<h3 style='text-align:center;'>$count resultats trouvés pour <strong>$search</strong></h3>";
  ...
  echo "<h3 style='text-align:center;'>aucun resultat trouvé pour <strong>$search</strong></h3>";
 }
   ?>
  <ul class="pager">
